...
public class PolicyAPI {
    ...
    public Response cancelPolicy(PolicyCancelRequest policyCancelRequest, @Context HttpServletRequest headers) {
        ...
        if (policyCancelRequest != null) {
            ...
            try {
                ...
                int updated = insurancePolicyInterface.updatePolicy(today(), setStatus(Statuses.CANCELLED.toString()), policyCancelRequest.getUsername(),
    ...
    @GET
    @Path("/check-collection-period")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response checkCollectionPeriod(@QueryParam("effectiveDate") String effectiveDate,
                                               @QueryParam("sessionId") String sessionId,@QueryParam("username") String username,@Context HttpServletRequest headers) {

        String reqRes = getLogId();
        String methodName = "checkCollectionPeriod";
        LOGGER.info("{} is being called with parameter. username -> {}, sessionId -> {}, logId -> {},  effectiveDate -. {}, ipAddress -> {} ",
                methodName, username, sessionId, reqRes,effectiveDate, headers.getRemoteAddr());
        ...
        try {
            ...
            boolean isAfterCollectionPeriod = false;
            if(application.isPresent()) {
                isAfterCollectionPeriod = isEffectiveDateAfterCollectionPeriod(stringToDate(effectiveDate),parseCollectionDays(application.get().getCollectionDays()));
                LOGGER.info("Is after collection period? {} , effective date {}", isAfterCollectionPeriod,stringToDate(effectiveDate));
            }
            response = Response.status(Response.Status.OK).entity(isAfterCollectionPeriod).build();
            defaultSuccess(LOGGER,reqRes);
            queryExecuted = true;
            ...
            try {
                ...
                for (BeneficiaryRequestPayload beneficiaries : benToBeRemoved) {
                    ...
                    int removed = beneficiariesInterface.updateBeneficiary(today(), policyUpdateRequest.getUsername(),
                            setStatus(Statuses.REMOVED.toString()),stringToDate(beneficiaries.getEffectiveDate()), beneficiaries.getBeneficiaryId());
                    if (removed > 0) {
        ...
        if (beneficiaryRequest == null) {
            ...
            try {
                ...
                if (beneficiaryRequest.getOperationType().equals(OperationTypes.REMOVE.toString())) {
                    ...
                    int updatedBeneficiary = beneficiariesInterface.updateBeneficiary(today(), beneficiaryRequest.getUsername(), status,today(),
    ...
    public Response reopenPolicy(PolicyReopenRequest policyReopenRequest,
        ...
        try {
            ...
            int updated = insurancePolicyInterface.updatePolicy(today(), setStatus(Statuses.ACTIVE.toString()), policyReopenRequest.getUsername(),
