...
import static core.constants.AttributeTypes.*;
...
public class ReportAPI {
    ...
    public Response getCommissionData(@QueryParam("startDate") String startDate,
        ...
        try {
            ...
            if(resultType.equalsIgnoreCase(ResultTypes.PREVIEW.toString())){
                ...
                response = buildCommissionFileForDownload(commissionResponses,username);
            }
            queryExecuted = true;
    ...
    private Response buildCommissionFileForDownload(List<CommissionResponse> commissionResponses,String username) throws Exception {
        ReportResponse reportResponse = new ReportResponse();



        LOGGER.info("Building Commission file for download ...");
        String[] columns = { "Número de apólice", "Produto","Sub Produto","CIF", "Nome do cliente",
                "Número de Conta" , "Dependentes","Capital Seguro", "Prémio total","Moeda", "Prémio Hollard", "SB Fee",
                "Net GPI", "Net Premium","Commission","ID da transacção"};


        if(commissionResponses.size()>0){
            String attributes = commissionResponses.get(0)
                    .getAttributes();

            if(attributes.contains(HAS_STAMP_DUTY.getAttribute())){
                columns = new String[]{"Número de apólice", "Produto", "Sub Produto", "CIF", "Nome do cliente",
                        "Número de Conta", "Dependentes","Capital Seguro", "Prémio total", "Moeda", "Prémio Hollard", "SB Fee",
                        "Stamp duty", "Net Premium", "Commission", "ID da transacção"};
            }
           /* if(attributes.contains(HASFIXEDRISKRATE.toString())){
                columns = new String[]{"Número de apólice", "Produto", "Sub Produto", "CIF", "Nome do cliente",
                        "Número de Conta", "Dependentes", "Capital Seguro", "Moeda", "Prémio Hollard", "SB Fee",
                        "Stamp duty", "Net Premium", "Commission", "ID da transacção"};
            } */
        }
        ...
        double _Commission;
        double _NetPremium;
        double premiumHollard = 0;

        for (CommissionResponse cr : commissionResponses) {

            try {
                InsurancePolicy ip = cr.getInsurancePolicy();

                Row row = sheet.createRow(rowNum++);
                row.createCell(0).setCellValue(ip.getPolicyId());
                row.createCell(1).setCellValue(cr.getProductName());
                row.createCell(2).setCellValue(cr.getSubProductName());
                row.createCell(3).setCellValue(cr.getCustomerId());
                row.createCell(4).setCellValue(cr.getCustomerName());
                row.createCell(5).setCellValue(cr.getAccountNumber());
                row.createCell(6).setCellValue(cr.getTotalChargeableDependents());


                try{
                    BigDecimal coverageAmount = cr.getCoverAmount();
                    BigDecimal annualPayroll = cr.getAnnualPayroll();


                        row.createCell(7).setCellValue(Util.formatDouble(coverageAmount.doubleValue()));
                ...
                if (ip.getCurrency() != null) {
                    row.createCell(9).setCellValue(ip.getCurrency().getCurrencyId());
                } else {
                    row.createCell(9).setCellValue("");
                }

                double _NetGPI = cr.getNetGPIRatio();
                double _NetCommissionPercentage = cr.getNetCommissionPercentage();
                double _SBFee = 0;
                try {
                    _SBFee = ip.getTotalAmount().doubleValue() - cr.getInsurerPremium();
                    premiumHollard = cr.getInsurerPremium();
                }
                ...
                /* if(cr.getNumberOfEmployees()!=null){
                    int numberOfEmployees = cr.getNumberOfEmployees().intValue();
                    Integer numberOfWorkers = productConfigInterface.findNumberOfWorkers(cr.getSubProductId().longValue(),setActive());

                    if(cr.getNumberOfEmployees().intValue()<
                            numberOfWorkers){
                        numberOfEmployees = numberOfWorkers;
                    }

                    premiumHollard = cr.getInsurerPremium()*numberOfEmployees;
                    _SBFee = (ip.getTotalAmount().doubleValue() - premiumHollard);
                } */

                if(cr.getSbFee()!=null){
                    _SBFee = cr.getSbFee().doubleValue();
                    premiumHollard = ip.getTotalAmount().doubleValue() - _SBFee;
                }

                if(cr.getAttributes().contains(HAS_FIXED_FEE.getAttribute())){
                    premiumHollard = ip.getTotalAmount().doubleValue();
                    _SBFee =ip.getTotalAmount().doubleValue() - premiumHollard;
                }


                if(cr.getAttributes().contains(HAS_FIXED_RISK_RATE.getAttribute())){
                    double baseRate = cr.getCustomFee();



                }

                if(cr.getAttributes().contains(HAS_NO_COMMISSION.getAttribute())){
                    try {

                        //premiumHollard = insuredCapital.doubleValue() * baseRate / 100;
                        _SBFee = 0;
                        row.createCell(8).setCellValue(Util.formatDouble((premiumHollard + _SBFee)));
                ...
                double _NetGPICalculated = premiumHollard/_NetGPI;

                if (cr.isHasStampDuty()) {
                    _Commission = ((_NetGPICalculated * _NetCommissionPercentage) / 100);
                } else {
                    _Commission = ((_NetGPICalculated * _NetCommissionPercentage) / 100) + _SBFee;
                }

              //  _NetPremium = premiumHollard - _Commission + _SBFee;
                _NetPremium = premiumHollard - _Commission;

                if(cr.getProductId().equals(Products.SALARY_PROTECT.toString())){
                    _NetPremium = premiumHollard - _Commission;
                }



                row.createCell(10).setCellValue(formatDouble(premiumHollard));
                row.createCell(11).setCellValue(formatDouble(_SBFee));
                row.createCell(12).setCellValue(formatDouble(_NetGPICalculated));
                row.createCell(13).setCellValue(formatDouble(_NetPremium));
                row.createCell(14).setCellValue(formatDouble(_Commission));
                row.createCell(15).setCellValue(cr.getTransactionId());
            }
            catch (Exception e){
                LOGGER.error(e);
            }
        ...
        // Write the output to a file
        String dateFormatted = formatDate(today(),TIMESTAMP_PATTERN);
        String url = reportDir+ File.separator + "Commission"+dateFormatted+".xlsx";
        FileOutputStream fileOut = new FileOutputStream(url);
        workbook.write(fileOut);
        fileOut.close();

        String shortUrl =  File.separator+ REPORT_DIR+ File.separator+ EXPORT_DIR+ File.separator+
                formatDate(today(),SIMPLE_DATE_PATTERN)+File.separator+username+File.separator + "Commission"+dateFormatted+".xlsx";
        LOGGER.info("File saved in path {}",url);

        reportResponse.setFile(FileUtil.toBase64(shortUrl));
        reportResponse.setFileName("Commission"+dateFormatted+".xlsx");
